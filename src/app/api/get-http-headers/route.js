...
import { GET_ALL_HTTP_HEADERS_PAGEABLE } from "../../database/schema";
...
export async function GET(req) {
  ...
  try {
    ...
    const httpHeadersData = await db.all(GET_ALL_HTTP_HEADERS_PAGEABLE, [
      case_uuid,
      limit,
      offset,
    ]);
    const totalRecordsResults = await db.get(
      "SELECT COUNT(*) FROM http_headers WHERE case_uuid = ?",
      [case_uuid],
    );
    const totalRecords = totalRecordsResults["COUNT(*)"];
    return new Response(
      JSON.stringify({
        data: httpHeadersData,
        page: page,
        limit: limit,
        total: totalRecords,
