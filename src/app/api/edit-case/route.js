...
export async function PUT(req) {
  ...
  const { case_number,case_uuid,case_investigator_name,case_investigator_organization,case_description } = await req.json();
  ...
  // Update the case details in the "case_details" table
    await db.run("UPDATE case_details SET case_number = ?, case_description = ?, case_investigator_name = ?, case_investigator_organization = ? WHERE case_uuid = ?", [
      case_number,
