...
class BarGraph extends Component<BarGraphProps> {
  constructor(props) {
    ...
    const httpRequestsData = props.httpRequestsData;

    // Extract the packet number and source IP address from the httpRequestsData array
    const packetData = httpRequestsData.map(item => ({
      x: item.sourceIp,
      y: item.packetNumber
    }));
